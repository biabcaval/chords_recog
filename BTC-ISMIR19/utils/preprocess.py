...
class Preprocess():
    ...
    def process_one_song_voca(self, song_tuple):
        """Process a single song using the legacy (pyrb) pipeline.

        Designed to be used as the worker function for
        ``multiprocessing.Pool.imap_unordered``, returning a status dict
        per song that the caller (e.g. :mod:`scripts.preprocess_decomposed`)
        can aggregate to drive a tqdm progress bar.

        Args:
            song_tuple: ``(song_name, lab_path, mp3_path, save_path)`` as
                returned by :meth:`get_all_files`. The ``save_path`` is
                appended with ``'_voca'`` to match the legacy layout.

        Returns:
            dict with the same shape as :meth:`process_one_song_cqtroll`'s
            return value: ``song_name``, ``song_failed``, ``instances_written``,
            ``instances_skipped``, optional ``reason``.
        """
        pid = os.getpid()
        mp3_config, feature_config, mp3_str, feature_str = self.config_to_folder()
        ...
        song_name, lab_path, mp3_path, save_path = song_tuple
        ...
        try:
            ...
            print(f"{pid} load error '{song_name}': {e}")
            return {
                'song_name': song_name, 'song_failed': True,
                'instances_written': 0, 'instances_skipped': 0,
        ...
        for stretch_factor in stretch_factors:
            for shift_factor in shift_factors:
                ...
                if audio_length + skip_interval_samples < last_sec_hz:
                    print(f"{pid} song too short '{song_name}'")
                    return {
                        'song_name': song_name, 'song_failed': True,
                        'instances_written': instances_written,
                        'instances_skipped': instances_skipped,
                ...
                while current_start_second + mp3_config['inst_len'] < origin_length_in_sec:
                    ...
                    while curSec < inst_start_sec + mp3_config['inst_len']:
                        try:
                            ...
                            bass = 12
                            chord_label = 'N'
                            print(f"{pid} chord-assign error '{song_name}': {e}")
                            raise RuntimeError()
                        finally:
                    ...
                    if len(chord_list) == self.no_of_chord_datapoints_per_sequence:
                        ...
                        if os.path.exists(output_file):
                            ...
                            try:
                                ...
                                if feature.shape[1] != self.no_of_chord_datapoints_per_sequence:
                                    print(f"{pid} loaded features length is too short '{song_name}'")
                                    return {
                                        'song_name': song_name, 'song_failed': True,
                                        'instances_written': instances_written,
                                        'instances_skipped': instances_skipped,
                                ...
                                print(f"{pid} feature error '{song_name}': {e}")
                                raise RuntimeError()
                    else:
                        print(f"{pid} invalid number of chord datapoints in sequence: {len(chord_list)}")
                    current_start_second += mp3_config['skip_interval']
    ...
    def generate_labels_features_voca(self, all_list):
        """Sequential wrapper around :meth:`process_one_song_voca`.

        Kept for backwards compatibility and non-parallel callers.  For
        parallel preprocessing with a live progress bar, use
        :meth:`process_one_song_voca` directly from a
        :class:`multiprocessing.Pool` (see
        :mod:`scripts.preprocess_decomposed`).
        """
        total_written = 0
        total_skipped = 0
        failed_songs = 0
        ...
        for song in all_list:
            result = self.process_one_song_voca(song)
            total_written += result['instances_written']
            total_skipped += result['instances_skipped']
            if result['song_failed']:
                failed_songs += 1

        print(
            f"{os.getpid()} pyrb done: "
            f"{total_written} instances written, "
            f"{total_skipped} skipped (already existed), "
            f"{failed_songs} songs failed"
        )

    def _validate_cqtroll_config(self):
        """Static-style validation of the cqtroll preconditions.

        Raised both up-front (so the user gets an immediate error before
        any worker starts) and defensively from :meth:`process_one_song_cqtroll`
        (so direct callers without going through the wrapper also fail fast).
        """
        feature_config = self.config.feature
        if feature_config['bins_per_octave'] % 12 != 0:
            raise ValueError(
                "cqtroll requires bins_per_octave to be a multiple of 12 "
                f"(got {feature_config['bins_per_octave']})"
            )

    def process_one_song_cqtroll(self, song_tuple):
        """Process a single song using the CQT-first (cqtroll) pipeline.

        Designed to be used as the worker function for
        ``multiprocessing.Pool.imap_unordered``, returning a status dict
        per song that the caller (e.g. :mod:`scripts.preprocess_decomposed_cqtroll`)
        can aggregate to drive a tqdm progress bar.
        ...
        Returns:
            dict with keys:
              - ``song_name``: str
              - ``song_failed``: bool (True if the song was skipped or
                aborted mid-way; False if it completed normally)
              - ``instances_written``: int (new ``.pt`` files written)
              - ``instances_skipped``: int (``.pt`` files already present
                and left untouched)
              - ``reason``: optional str when ``song_failed`` is True
        """
        self._validate_cqtroll_config()
        ...
        try:
            ...
            print(f"{pid} full-song cqt error '{song_name}': {e}")
            return {
                'song_name': song_name, 'song_failed': True,
                'instances_written': instances_written,
                'instances_skipped': instances_skipped,
                'reason': f'cqt_error: {e}',
            }
    ...
    def generate_labels_features_voca_cqtroll(self, all_list):
        ...
        This is a thin sequential wrapper around
        :meth:`process_one_song_cqtroll`, kept for backwards compatibility
        and for non-parallel callers.  For parallel preprocessing with a
        live progress bar, use :meth:`process_one_song_cqtroll` directly
        from a :class:`multiprocessing.Pool` (see
        :mod:`scripts.preprocess_decomposed_cqtroll`).
        """
        self._validate_cqtroll_config()
        ...
        for song in all_list:
            result = self.process_one_song_cqtroll(song)
            total_written += result['instances_written']
            total_skipped += result['instances_skipped']
            if result['song_failed']:
                failed_songs += 1
